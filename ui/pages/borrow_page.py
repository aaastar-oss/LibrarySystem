...
class BorrowPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        borrow_btn.pack(pady=10, ipady=2)
        ...
        # 按钮悬停效果
        borrow_btn.bind("<Enter>", lambda e: borrow_btn.config(bg="#3a56b0"))
        borrow_btn.bind("<Leave>", lambda e: borrow_btn.config(bg=controller.PRIMARY_COLOR))
        ...
        # 绑定回车键
        self.book_id_entry.bind("<Return>", lambda e: self.do_borrow())
    ...
    def do_borrow(self):
        ...
        if not book_id:
            ...
            self.controller.set_status("借书失败：未输入编号", color=self.controller.DANGER_COLOR)
            return
            
        if not book_id.isdigit():
            self.status_label.config(text="图书编号必须是数字", fg=self.controller.DANGER_COLOR)
            self.controller.set_status("借书失败：编号格式错误", color=self.controller.DANGER_COLOR)
            return
